...
// ── Fan comment personality styles (5 distinct voices) ────────────
const FAN_STYLES = [
  { label: "따뜻",   system: "당신은 따뜻하고 다정한 팬이에요. 진심 어린 응원과 따뜻한 말로 댓글을 남겨요. 2문장 이내, 이모지 1~2개." },
  { label: "순덕",   system: "당신은 순수하고 덕질에 진심인 팬이에요. 조금 흥분되고 귀엽게 리액션해요. 2문장 이내, 이모지 2~3개." },
  { label: "열성팬", system: "당신은 열성팬이에요. 과장되게 감동받고 최고라며 극찬해요. 느낌표 많이, 2문장 이내, 이모지 2~3개." },
  { label: "츤데레", system: "당신은 츤데레 팬이에요. 칭찬하면서도 '뭐 그냥...' 식으로 쑥스러워해요. 2문장 이내, 이모지 0~1개." },
  { label: "얌전",   system: "당신은 조용하고 얌전한 팬이에요. 짧고 소박하지만 진심이 담긴 응원을 남겨요. 1~2문장, 이모지 1개." },
];

// ── Auto fan-letter templates for the board ───────────────────────
const LETTER_TEMPLATES = [
  (name) => `안녕 ${name}!! 오늘도 블로그 보러 왔어~ 매일 이렇게 열심히 기록하는 거 너무 멋있어 ✨ 나도 본받아야 하는데ㅠㅠ 응원해!!`,
  (name) => `${name}야 밥은 먹었어?? 오늘 날씨 좋던데 기분도 좋았으면 좋겠어 🌷 항상 네 공간이 너무 예뻐서 자꾸 오게 돼 💙`,
  (name) => `팬레터 처음 써보는데 너무 떨린다ㅠ ${name} 존재 자체가 힐링이야 진짜로... 오늘도 수고했어요 🌙`,
  (name) => `${name} 오늘 뽀모도로 몇 개 했어?? 나도 같이 집중하고 싶어서 여기 왔어ㅎㅎ 같이 화이팅하자!! 💪`,
  (name) => `이 플래너 진짜 너무 예쁘다ㅠㅠㅠ ${name}이 만든 공간이라 더 그런 것 같아 🌸 매일 보러올게!!`,
  (name) => `${name}아 오늘 루틴 다 챙겼어? 비타민 먹는 거 잊지 마!! 건강이 제일 중요해 🌿 사랑해~`,
  (name) => `새벽에 여기 와서 ${name} 일기 읽었어... 나만 알고 싶은 공간이다ㅠ 앞으로도 자주 적어줘!! 📖`,
  (name) => `${name}은 왜 이렇게 열심히 살아?? 나도 자극받고 가야지 ㅠㅠ 오늘도 수고했고 내일도 화이팅!! ⭐`,
];

// ── Guestbook message templates ───────────────────────────────────
...
function generateDailyMessages(profileName, date, fans) {
  ...
  return Array.from({ length: 10 }, (_, i) => {
    const fan = fans[Math.floor(rng() * fans.length)];
    const msg = MESSAGE_TEMPLATES[Math.floor(rng() * MESSAGE_TEMPLATES.length)].replace(/hyeoni/g, profileName);
    const minAgo = Math.floor(rng() * 480);
    const time = new Date(date);
    time.setHours(8 + Math.floor(minAgo / 60), minAgo % 60, 0, 0);
    return { id: `gb-auto-${seed}-${i}`, fan, msg, time };
  }).sort((a, b) => b.time - a.time);
}

// Daily fan letters posted automatically to the board (3 per day)
function generateDailyLetters(profileName, date, fans) {
  const seed = (date.getFullYear() * 10000 + (date.getMonth() + 1) * 100 + date.getDate()) ^ 0xdeadbeef;
  const rng = seededRng(seed);
  return Array.from({ length: 3 }, (_, i) => {
    const fan = fans[Math.floor(rng() * fans.length)];
    const tmpl = LETTER_TEMPLATES[Math.floor(rng() * LETTER_TEMPLATES.length)];
    const text = tmpl(profileName);
    const minAgo = Math.floor(rng() * 600);
    const time = new Date(date);
    time.setHours(7 + Math.floor(minAgo / 60), minAgo % 60, 0, 0);
    const virtualComments = 97 + Math.floor(rng() * 24); // ~100+ feel
    return {
      id: `letter-auto-${seed}-${i}`,
      type: "fan",
      authorName: fan.name,
      authorEmoji: fan.emoji,
      authorColor: fan.color,
      text,
      photo: null,
      time,
      comments: [],
...
function useTimer(focusMins, breakMins) {
  ...
  useEffect(() => {
    if (phase === "idle" || phase === "paused") { clearInterval(ref.current); return; }
    ref.current = setInterval(() => {
      setRemaining(r => {
        if (r <= 1) { clearInterval(ref.current); const next = !isFocus; setIsFocus(next); setPhase("idle"); return next ? breakMins * 60 : focusMins * 60; }
        return r - 1;
      });
    }, 1000);
  ...
  const reset = () => { clearInterval(ref.current); setPhase("idle"); setIsFocus(true); setRemaining(focusMins * 60); };
...
function readFileAsDataURL(file, cb) { if (!file) return; const r = new FileReader(); r.onload = () => cb(r.result); r.readAsDataURL(file); }

const INIT_BOARD_POSTS = [
  {
    id: 1, type: "owner", authorName: "hyeoni", authorEmoji: "👑", authorColor: "#DFF0FA",
    text: "게시판을 새로 만들었어요! 하고 싶은 말이나 편지, 자유롭게 남겨주세요 🩵",
    photo: null, time: new Date(Date.now() - 1000 * 60 * 60 * 20),
    comments: [
      { id: 101, name: "별빛토끼", text: "드디어 게시판 생겼다!! 너무 좋아 🐰", time: new Date(Date.now() - 1000 * 60 * 60 * 18), isOwner: false },
    ],
    virtualComments: 143, isAutoLetter: false,
  },
];
...
export default function HyeoniSpace() {
  ...
  const startEditFan = (idx, name) => { setEditingFanIdx(idx); setFanNameDraft(name); };
  const saveFanName = (idx) => { const t = fanNameDraft.trim(); if (t) setFans(p => p.map((f,i) => i===idx ? {...f,name:t} : f)); setEditingFanIdx(null); };

  // Auto daily fan letters (resets to a fresh batch every new day)
  const [autoLetters, setAutoLetters] = useState([]);
  const [lastLetterDate, setLastLetterDate] = useState(todayKey);
  useEffect(() => {
    setAutoLetters(generateDailyLetters(profileName, now, fans));
  }, [profileName, fans]);
  useEffect(() => {
    if (todayKey !== lastLetterDate) {
      setAutoLetters(generateDailyLetters(profileName, now, fans));
      setLastLetterDate(todayKey);
    }
  }, [todayKey]);

  const [manualPosts, setManualPosts] = useState(INIT_BOARD_POSTS);
  const [nextBoardId, setNextBoardId] = useState(2);
  const [composerMode, setComposerMode] = useState("owner");
  ...
  const [generatingCommentFor, setGeneratingCommentFor] = useState(null);

  // All board posts = manual + today's auto letters, sorted by time
  const boardPosts = [...manualPosts, ...autoLetters].sort((a,b) => new Date(b.time) - new Date(a.time));
  const totalComments = boardPosts.reduce((a,p) => a + p.comments.length + (p.virtualComments||0), 0);

  const addBoardPost = () => {
    const t = boardInput.trim();
    if (!t && !boardPhoto) return;
    const author = composerMode === "owner" ? profileName : (boardAuthorName.trim() || "익명의 팬");
    const emoji = composerMode === "owner" ? "👑" : "💌";
    const color = composerMode === "owner" ? "#DFF0FA" : "#FFE8F4";
    // 실제로 보이는 진짜 댓글은 최대 5개지만, 댓글 수는 100+로 보이도록 가짜 카운트를 바로 부여
    const initialVirtual = composerMode === "owner" ? 100 + Math.floor(Math.random() * 60) : 0;
    const newPost = {
      id: nextBoardId, type: composerMode, authorName: author, authorEmoji: emoji, authorColor: color,
      text: t, photo: boardPhoto, time: new Date(), comments: [], virtualComments: initialVirtual, isAutoLetter: false,
    };
    setManualPosts(p => [newPost, ...p]);
    ...
    // 내가 쓴 글에만 팬들이 자동으로 댓글을 달아줌 (5가지 다른 성격의 팬)
    if (composerMode === "owner") {
      generateFanComments(newPost.id, t);
    }
  };

  // 5명의 팬이 서로 다른 성격(따뜻/순덕/열성팬/츤데레/얌전)으로 한 명씩 차례로 댓글을 남김
  const generateFanComments = async (postId, postText) => {
    setGeneratingCommentFor(postId);
    const shuffledFans = [...fans].sort(() => Math.random() - 0.5);
    try {
      for (let i = 0; i < 5; i++) {
        const fan = shuffledFans[i % shuffledFans.length];
        const style = FAN_STYLES[i];
        try {
          const res = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
              ...
              max_tokens: 300,
              system: style.system + " 반말 사용. 댓글 내용만 출력하고 다른 말은 하지 마.",
              messages: [{
                role: "user",
                content: `블로그 주인 "${profileName}"이 방금 이런 글을 올렸어: "${postText.slice(0, 150)}"\n\n이 글에 짧은 댓글을 한 개만 남겨줘.`
              }]
            })
          });
          ...
          if (text) {
            const comment = { id: Date.now() + i, name: fan.name, fanEmoji: fan.emoji, styleLabel: style.label, text, time: new Date(), isOwner: false };
            setManualPosts(prev => prev.map(p => p.id === postId ? { ...p, comments: [...p.comments, comment] } : p));
          }
        } catch { /* 개별 댓글 실패는 조용히 건너뜀 */ }
      }
    } finally {
  ...
  const deleteBoardPost = (id) => setManualPosts(p => p.filter(post => post.id !== id));
  const startEditPost = (post) => { setEditingPostId(post.id); setEditingPostText(post.text); };
  const saveEditPost = (id) => { setManualPosts(p => p.map(post => post.id===id ? {...post,text:editingPostText.trim()} : post)); setEditingPostId(null); };
  ...
  const addComment = (postId) => {
    ...
    setManualPosts(p => p.map(post => post.id===postId ? {...post,comments:[...post.comments,{id:Date.now(),name,text,time:new Date(),isOwner:false}]} : post));
  ...
  const addOwnerReply = (postId, commentId) => {
    ...
    setManualPosts(p => p.map(post => post.id===postId ? {...post,comments:[...post.comments,{id:Date.now(),name:profileName,text:t,time:new Date(),isOwner:true,parentId:commentId}]} : post));
    setReplyDraftFor(null); setReplyText("");
  };
  const deleteComment = (postId, commentId) => setManualPosts(p => p.map(post => post.id===postId ? {...post,comments:post.comments.filter(c=>c.id!==commentId)} : post));
  ...
  useEffect(() => { setAiMessages(generateDailyMessages(profileName, now, fans)); }, [profileName, fans]);
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role:"user", content:`당신은 "${fan.name}"이라는 귀여운 팬이에요. "${profileName}"라는 사람의 방명록에 귀엽고 따뜻한 한국어 응원 메시지를 1개 남겨주세요. 2~3문장, 이모지 2~3개, 반말, 메시지만 출력.` }]
        })
      });
      const data = await res.json();
      const msg = data.content?.[0]?.text?.trim() || "오늘도 화이팅!! 💕";
      setAiMessages(prev => [{id:`gen-${Date.now()}`,fan,msg,time:new Date()},...prev]);
      setGbNotif(`${fan.emoji} ${fan.name}님이 방명록을 남겼어요!`);
      setTimeout(() => setGbNotif(null), 3000);
    } catch { setGbNotif("메시지를 불러오지 못했어요 😢"); setTimeout(()=>setGbNotif(null),2000); }
    setIsGenerating(false);
  };

  const sendGuestbook = () => {
    const t = gbInput.trim(); if (!t) return;
    const name = gbName.trim() || "익명의 방문자";
    setGbEntries(prev => [{id:`user-${Date.now()}`,fan:{name,emoji:"✍️",color:"#F0F8FF"},msg:t,time:new Date()},...prev]);
    setGbInput(""); setGbName("");
  };
  ...
  const savePanelEdit = (pid) => { const t=panelEditText.trim(); if(t) setPanelItems(p=>({...p,[pid]:p[pid].map((it,i)=>i===panelEditIdx?t:it)})); setPanelEditIdx(null); };
  ...
  const saveEditText = (id) => { const t=editTextDraft.trim(); if(t) setItems(p=>p.map(i=>i.id===id?{...i,text:t}:i)); setEditingTextId(null); };
  ...
  const saveCalEdit=(k)=>{ const t=calEditText.trim(); if(t) setCalNotes(p=>({...p,[k]:p[k].map((n,i)=>i===calEditIdx?{...n,text:t}:n)})); setCalEditIdx(null); };
  ...
  const saveDiaryEdit=(id)=>{ const t=editingDiaryText.trim(); setDiaryEntries(p=>p.map(e=>e.id===id?{...e,text:t}:e)); setEditingDiaryId(null); };
  ...
  const formatGbTime=(t)=>{ const d=new Date(t); return `${d.getMonth()+1}/${d.getDate()} ${pad(d.getHours())}:${pad(d.getMinutes())}`; };
  ...
  return (
    <div style={s.page}>
      <style>{`
        ...
        .hp-scroll::-webkit-scrollbar{width:6px;}.hp-scroll::-webkit-scrollbar-thumb{background:#B8DCEF;border-radius:4px;}
        .hp-navbtn:hover{background:#CDE9F8 !important;}.hp-navbtn.active:hover{background:#7EC8E3 !important;}
        .hp-photobtn:hover{background:#F4A7C3 !important;color:#fff !important;}
        .gb-gen-btn:hover{opacity:.85;transform:scale(1.02);}
        @keyframes fanPop{0%{opacity:0;transform:translateY(12px) scale(.95)}100%{opacity:1;transform:translateY(0) scale(1)}}
        .gb-fan-entry{animation:fanPop .35s ease;}
        @keyframes notifSlide{0%{opacity:0;transform:translateY(-20px)}100%{opacity:1;transform:translateY(0)}}
        .gb-notif{animation:notifSlide .3s ease;}
        @keyframes shimmer{0%,100%{opacity:1}50%{opacity:.4}}.gen-shimmer{animation:shimmer 1.2s infinite;}
      ...
      {gbNotif && (
        <div className="gb-notif" style={{position:"fixed",top:20,right:20,background:"#fff",border:"1.5px solid #F4A7C3",borderRadius:12,padding:"10px 16px",fontSize:13,color:"#C06090",boxShadow:"0 4px 16px rgba(244,167,195,.3)",zIndex:9999,fontFamily:"monospace"}}>
          {gbNotif}
        </div>
      )}
        ...
        <div className="hp-body" style={s.body}>
          ...
          <aside className="hp-sidebar" style={s.sidebar}>
            <div className="hp-sidebar-profile" style={s.profileBox}>
              ...
              <button onClick={()=>profilePhotoRef.current?.click()} style={{...s.profileBadge,backgroundImage:profilePhoto?`url(${profilePhoto})`:undefined}}>
                {!profilePhoto && <PixelHeart color="#fff" size={32}/>}
              </button>
              <div style={{minWidth:0,flex:1}}>
                <div style={{display:"flex",alignItems:"baseline",gap:6,flexWrap:"wrap"}}>
                  {editingName ? <input autoFocus value={nameDraft} onChange={e=>setNameDraft(e.target.value)} onBlur={saveName} onKeyDown={e=>e.key==="Enter"&&saveName()} style={s.nameInput}/> : <div style={s.profileName} onClick={startEditName}>{profileName} ♡ ✏️</div>}
                  {editingTheme ? <input autoFocus value={themeDraft} onChange={e=>setThemeDraft(e.target.value)} onBlur={saveTheme} onKeyDown={e=>e.key==="Enter"&&saveTheme()} style={s.themeInput} placeholder="테마 음악"/> : <span style={s.themeTag} onClick={startEditTheme}>♪ {themeMusic}</span>}
                </div>
            ...
            <nav className="hp-navlist" style={s.navList}>
              {PAGES.map(p=>(
                <button key={p.id} className={`hp-navbtn${page===p.id?" active":""}`} onClick={()=>setPage(p.id)}
                  ...
                  <span style={{opacity:.8,marginRight:6}}>{p.mark}</span>{p.label}
            ...
            <div style={s.sidebarDivider}/>
            <div className="hp-shelf" style={s.shelfRow}><PixelLamp/><PixelPlant/><PixelBook/><PixelFrame/></div>
                ...
                {aiMessages[0] && (
                  <div style={{...s.homeDiaryPreview,borderLeft:"3px solid #F4A7C3"}}>
                    ...
                    <div style={s.quoteCard}>
                      <div style={{fontSize:11,color:"#C097B0",marginBottom:4}}>{aiMessages[0].fan.emoji} {aiMessages[0].fan.name}</div>
              ...
              <div style={s.routineWrap}>
                <div style={s.card}>
                  <div style={s.cardBar}>
                    ...
                    <button onClick={()=>setShowRoutineHistory(v=>!v)} style={s.closeBtn}>📜</button>
                  </div>
                  {showRoutineHistory && (
                        ...
                        <div className="hp-scroll" style={s.panelList}>
                          ...
                          {list.map((item,idx)=>(
                            <div key={idx} style={s.panelRow}>
                              ...
                              {panelEditIdx===idx ? (
                                <input autoFocus value={panelEditText} onChange={e=>setPanelEditText(e.target.value)} onBlur={()=>savePanelEdit(activePanel)} onKeyDown={e=>e.key==="Enter"&&savePanelEdit(activePanel)} style={{...s.textInput,flex:1,padding:"3px 6px",fontSize:13}}/>
                              ) : (
                                <span style={{...s.panelText,textDecoration:checked.includes(idx)?"line-through":"none",color:checked.includes(idx)?"#A0C8DC":"#3A7A9C",cursor:"pointer"}} onClick={()=>startPanelEdit(idx,item)}>{item}</span>
                              )}
                              {panelEditIdx!==idx && <button onClick={()=>startPanelEdit(idx,item)} style={s.editBtn}>✏️</button>}
                    ...
                    <div className="hp-scroll" style={{maxHeight:140,overflowY:"auto"}}>
                      {(calNotes[calKey(selectedDay)]||[]).map((n,i)=>(
                        <div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                          ...
                          {calEditIdx===i ? (
                            <input autoFocus value={calEditText} onChange={e=>setCalEditText(e.target.value)} onBlur={()=>saveCalEdit(calKey(selectedDay))} onKeyDown={e=>e.key==="Enter"&&saveCalEdit(calKey(selectedDay))} style={{...s.textInput,flex:1,fontSize:12,padding:"3px 6px"}}/>
                          ) : (
                            <span style={{flex:1,fontSize:12,color:n.done?"#A0C0D8":"#3A6080",textDecoration:n.done?"line-through":"none",cursor:"pointer"}} onClick={()=>startCalEdit(i,n.text)}>{n.text}</span>
                          )}
                          {calEditIdx!==i && <button onClick={()=>startCalEdit(i,n.text)} style={s.editBtn}>✏️</button>}
            ...
            {page==="memo" && (
              <div style={s.card}>
                  ...
                  {filtered.map(item=>(
                    <div key={item.id} style={s.itemRow}>
                      ...
                      {editingTextId===item.id ? (
                        <input autoFocus value={editTextDraft} onChange={e=>setEditTextDraft(e.target.value)} onBlur={()=>saveEditText(item.id)} onKeyDown={e=>e.key==="Enter"&&saveEditText(item.id)} style={{...s.textInput,flex:1,fontSize:13,padding:"3px 6px"}}/>
                      ) : (
                        <span style={{...s.itemText,textDecoration:item.done?"line-through":"none",color:item.done?"#A0C0D8":"#3A6080",cursor:"pointer"}} onClick={()=>startEditText(item)}>{item.text}</span>
                      )}
                      {editingId===item.id ? (
                        ...
                        <span style={s.catLabel} onClick={()=>{setEditingId(item.id);setEditCat(item.category);}}>{item.category} ✏️</span>
                      )}
                ...
                <div style={s.inputRow}>
                  ...
                  <button className="hp-photobtn" onClick={()=>memoFileRef.current?.click()} style={s.photoBtn}>📷</button>
            ...
            {page==="diary" && (
              <div style={s.card}>
                ...
                <div style={s.composerRow}>
                  ...
                  <button className="hp-photobtn" onClick={()=>diaryFileRef.current?.click()} style={{...s.photoBtn,alignSelf:"flex-end"}}>📷</button>
                  ...
                  {diaryEntries.map(e=>(
                    <div key={e.id} style={s.diaryItem}>
                      <div style={{flex:1}}>
                        ...
                        {editingDiaryId===e.id ? (
                          <textarea autoFocus value={editingDiaryText} onChange={ev=>setEditingDiaryText(ev.target.value)} onBlur={()=>saveDiaryEdit(e.id)} style={{...s.diaryTextarea,width:"100%"}} rows={2}/>
                        ) : (
                          e.text && <div style={s.diaryText} onClick={()=>startDiaryEdit(e)}>{e.text}</div>
                        )}
                      </div>
                      {editingDiaryId!==e.id && <button onClick={()=>startDiaryEdit(e)} style={s.editBtn}>✏️</button>}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={s.card}>
                  ...
                  <div style={{padding:"10px 12px",display:"flex",flexDirection:"column",gap:8}}>
                    ...
                    {composerMode==="fan" && <input style={{...s.textInput,fontSize:12}} placeholder="이름 (비워두면 익명의 팬)" value={boardAuthorName} onChange={e=>setBoardAuthorName(e.target.value)}/>}
                    ...
                    <div style={{display:"flex",gap:8,justifyContent:"flex-end",alignItems:"center"}}>
                      {composerMode==="owner" && <span style={{fontSize:11,color:"#90BFDC",flex:1}}>✨ 글을 올리면 팬들이 자동으로 댓글을 달아줘요!</span>}
                      ...
                      <button className="hp-photobtn" onClick={()=>boardFileRef.current?.click()} style={s.photoBtn}>📷</button>
                ...
                <div style={s.card}>
                  <div style={s.cardBar}>
                    <span style={s.dot}/>
                    <span style={s.cardBarTitle}>게시글 {boardPosts.length}개</span>
                    <span style={{fontSize:11,color:"#F4A7C3",fontWeight:700,marginLeft:4}}>💬 {totalComments}+ 댓글</span>
                  </div>
                  <div className="hp-scroll" style={{padding:"10px 12px",display:"flex",flexDirection:"column",gap:10,maxHeight:560,overflowY:"auto"}}>
                    {boardPosts.length===0 && <div style={s.empty}>아직 게시물이 없어요 ✏️</div>}
                    {boardPosts.map(post=>{
                      const visibleCommentCount = post.comments.length + (post.virtualComments||0);
                      return (
                        <div key={post.id} style={s.boardPostCard}>
                          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                            <span style={{fontSize:18}}>{post.authorEmoji||"💌"}</span>
                            ...
                            {!post.isAutoLetter && <button onClick={()=>deleteBoardPost(post.id)} style={s.deleteBtn}>×</button>}
                          </div>
                          ...
                          {editingPostId===post.id ? (
                            ...
                            post.text && <div style={s.diaryText} onClick={()=>!post.isAutoLetter&&startEditPost(post)}>{post.text}</div>
                          )}
                          {/* Comment toggle + count */}
                          <button onClick={()=>setExpandedPostId(expandedPostId===post.id?null:post.id)} style={s.homeLinkBtn}>
                            💬 댓글 {visibleCommentCount}개 {expandedPostId===post.id?"숨기기":"보기"}
                          </button>
                          {/* Generating indicator */}
                          {generatingCommentFor===post.id && (
                            <div className="gen-shimmer" style={{fontSize:11,color:"#F4A7C3",marginTop:4}}>✨ 팬들이 댓글을 달고 있어요...</div>
                          )}
                          {/* Visible comments (max 5 real ones shown) */}
                          {expandedPostId===post.id && (
                            <div style={s.commentBox}>
                              {post.comments.length===0 && !post.isAutoLetter && generatingCommentFor!==post.id && <div style={{fontSize:11.5,color:"#A0C0D8"}}>아직 댓글이 없어요</div>}
                              {post.comments.slice(0,5).map(c=>(
                                <div key={c.id} style={{...s.commentRow,marginLeft:c.parentId?16:0,background:c.isOwner?"#FFF0F8":"#F0F8FF",borderColor:c.isOwner?"#F4A7C3":"#C8E4F4"}}>
                                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                                    {c.fanEmoji && <span>{c.fanEmoji}</span>}
                                    <span style={{fontSize:11.5,fontWeight:700,color:c.isOwner?"#C06090":"#4A9ABF"}}>{c.isOwner?"👑 ":""}{c.name}</span>
                                    {c.styleLabel && <span style={{fontSize:9,color:"#90BFDC",background:"#E8F4FF",borderRadius:6,padding:"1px 5px"}}>{c.styleLabel}</span>}
                              ...
                              {post.virtualComments>0 && (
                                <div style={{fontSize:11,color:"#B8DCEF",textAlign:"center",padding:"4px 0",borderTop:"1px dashed #D8EEF8",marginTop:4}}>
                                  ... 그 외 {post.virtualComments}개의 댓글이 더 있어요 💬
                                </div>
                              )}
                              {!post.isAutoLetter && (
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                <div style={{background:"linear-gradient(135deg,#FFE8F4,#F0E8FF)",border:"1.5px solid #F4A7C3",borderRadius:14,padding:"16px 18px",textAlign:"center"}}>
                  ...
                  <div style={{marginTop:10,display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
                    {(showAllFans?fans:fans.slice(0,5)).map((f,idx)=>(
                      editingFanIdx===idx ? (
                        <input key={idx} autoFocus value={fanNameDraft} onChange={e=>setFanNameDraft(e.target.value)} onBlur={()=>saveFanName(idx)} onKeyDown={e=>e.key==="Enter"&&saveFanName(idx)}
                        ...
                        <div key={f.name+idx} onClick={()=>startEditFan(idx,f.name)} title="클릭해서 이름 변경"
                          ...
                          {f.emoji} {f.name} <span style={{opacity:.6}}>✏️</span>
                        </div>
                      )
                    ))}
                    {!showAllFans && fans.length>5 && <button onClick={()=>setShowAllFans(true)} style={{background:"#F8F0FF",border:"1px solid #E0C8F4",borderRadius:20,padding:"3px 10px",fontSize:11,color:"#9060C0",cursor:"pointer"}}>+{fans.length-5}명 더</button>}
                    {showAllFans && <button onClick={()=>setShowAllFans(false)} style={{background:"#F8F0FF",border:"1px solid #E0C8F4",borderRadius:20,padding:"3px 10px",fontSize:11,color:"#9060C0",cursor:"pointer"}}>접기</button>}
                  </div>
                </div>
                <div style={{background:"#FFF0F8",border:"1.5px dashed #F4A7C3",borderRadius:12,padding:"14px 16px",textAlign:"center"}}>
                  ...
                  <button className="gb-gen-btn" onClick={generateNewMessage} disabled={isGenerating}
                    style={{background:isGenerating?"#E0C8D8":"linear-gradient(135deg,#F4A7C3,#C8A0E8)",color:"#fff",border:"none",borderRadius:20,padding:"10px 24px",fontSize:13,fontWeight:700,cursor:isGenerating?"not-allowed":"pointer",boxShadow:"0 3px 10px rgba(244,167,195,.4)",transition:"all .15s",letterSpacing:.5}}>
                  ...
                  <div className="hp-scroll" style={{padding:"10px 12px",maxHeight:420,overflowY:"auto",display:"flex",flexDirection:"column",gap:8}}>
                    ...
                    {allGbEntries.map(entry=>(
                      <div key={entry.id} className="gb-fan-entry"
                        style={{display:"flex",gap:10,padding:"10px 12px",background:entry.fan.color||"#F8FCFF",border:"1.5px solid #F4A7C3",borderRadius:12,alignItems:"flex-start"}}>
                        <div style={{width:32,height:32,borderRadius:"50%",background:"linear-gradient(135deg,#F4A7C3,#C8A0E8)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,border:"2px solid #fff",boxShadow:"0 2px 6px rgba(244,167,195,.3)"}}>
                          {entry.fan.emoji}
                        </div>
...
const s = {
  ...
  frame:{width:"100%",maxWidth:1040,background:"#F8FCFF",border:"1.5px solid #B8DCEF",borderRadius:16,overflow:"hidden",boxShadow:"0 10px 36px rgba(80,150,200,.18)",display:"flex",flexDirection:"column"},
  ...
  profileBadge:{width:58,height:58,borderRadius:16,background:"linear-gradient(135deg,#7EC8E3,#A0B4E8)",backgroundSize:"cover",backgroundPosition:"center",border:"none",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 3px 9px rgba(100,160,210,.35)",cursor:"pointer",padding:0},
  ...
  navBtn:{textAlign:"left",border:"none",borderRadius:8,padding:"8px 10px",fontSize:12.5,cursor:"pointer",letterSpacing:.3,transition:"background .15s",display:"flex",alignItems:"center"},
  ...
  card:{background:"#F8FCFF",border:"1.5px solid #B8DCEF",borderRadius:14,overflow:"hidden",boxShadow:"0 3px 14px rgba(100,180,220,.10)"},
  ...
  cardBarTitle:{fontSize:12,color:"#5A9FBF",flex:1,letterSpacing:.3},
  ...
  iconBtn:{width:34,height:34,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all .15s"},
  ...
  panelRow:{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,.7)",borderRadius:8,padding:"6px 9px",border:"1px solid #C8E4F4"},
  ...
  tab:{padding:"5px 9px",fontSize:11,fontWeight:600,border:"none",background:"transparent",color:"#90BFDC",cursor:"pointer",borderRadius:"6px 6px 0 0",letterSpacing:.3},
  ...
  photoBtn:{background:"#F0F8FF",color:"#5A9FBF",border:"1px solid #C8E4F4",borderRadius:8,padding:"6px 9px",fontSize:13,cursor:"pointer",flexShrink:0,transition:"all .15s"},
  ...
  lightboxOverlay:{position:"fixed",inset:0,background:"rgba(20,30,50,.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:24},
  lightboxImg:{maxWidth:"90vw",maxHeight:"90vh",borderRadius:10,boxShadow:"0 10px 40px rgba(0,0,0,.4)"},
